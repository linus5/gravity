...
func NewHandler(cfg WebHandlerConfig) *WebHandler {
	...
	wh.Handle("GET", "/web/installer/site/:site_domain/*rest", wh.needsLogin(wh.installerHandler))
...
func (h *WebHandler) rootHandler(w http.ResponseWriter, r *http.Request, p httprouter.Params, s session) {
	...
	if site != nil && h.cfg.Mode == constants.ComponentSite {
		http.Redirect(w, r, fmt.Sprintf("/web/site/%v", site.Domain), http.StatusFound)
		return
	}
